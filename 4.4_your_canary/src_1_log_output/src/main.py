...
def healthz():
    ...
    try:
        ...
        if response.status_code == 200:
            return JSONResponse(status_code=200, content={"status": "ready", "pingpong": "reachable"})
        else:
            return JSONResponse(status_code=503, content={"status": "not ready", "pingpong": "unreachable"})
    except Exception as e:
        return JSONResponse(status_code=503, content={"status": "not ready", "pingpong": "unreachable", "error": str(e)})
